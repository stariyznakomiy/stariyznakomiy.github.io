    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
                if (bodyLockStatus && e.target.closest(".icon-menu")) {
                    ...
                    const subMenuActive = document.querySelector(".menu__sub-menu_active");
                    bodyLockToggle();
                    ...
                    if (subMenuActive) subMenuActive.classList.remove("menu__sub-menu_active");
                }
                if (!e.target.closest(".header__mob") && !e.target.closest(".icon-menu") && !e.target.closest(".menu__body")) {
                    const menuBodyActive = document.querySelector(".menu__body._active");
                    const menuLinkActive = document.querySelector(".menu__link_active");
                    const subMenuActive = document.querySelector(".menu__sub-menu_active");
                    bodyUnlock();
        ...
        function documentActionsClick(e) {
            ...
            if (el.closest(".menu__link")) {
                ...
                const submenuActive = document.querySelector(".menu__sub-menu_active");
                if (isMobile.any()) if (submenu) {
                    if (activeLink && activeLink !== link) {
                        activeLink.classList.remove("menu__link_active");
                        submenuActive.classList.remove("menu__sub-menu_active");
                    }
                    link.classList.toggle("menu__link_active");
                    submenu.classList.toggle("menu__sub-menu_active");
                    e.preventDefault();
                }
            }
            if (!el.closest(".menu__link") && !el.closest(".menu__sub-menu")) if (isMobile.any()) {
                const activeLink = document.querySelector(".menu__link_active");
                const submenuActive = document.querySelector(".menu__sub-menu_active");
                if (activeLink) activeLink.classList.remove("menu__link_active");
                if (submenuActive) submenuActive.classList.remove("menu__sub-menu_active");
            }
            if (el.closest(".menu__back")) {
                const back = el.closest(".menu__back");
                const menuBody = back.closest(".menu__body");
                if (menuBody.classList.contains("_active")) menuBody.classList.toggle("_active");
            }
            if (el.closest(".sub-menu__back")) {
                const back = el.closest(".sub-menu__back");
                const subMenu = back.closest(".sub-menu");
                const link = subMenu.previousElementSibling;
                if (subMenu.classList.contains("menu__sub-menu_active")) {
                    subMenu.classList.remove("menu__sub-menu_active");
                    link.classList.remove("menu__link_active");
                }
            }
            if (el.closest(".search-header__icon")) {
                const button = el.closest(".search-header__icon");
                button.classList.toggle("_active");
                button.nextElementSibling.classList.toggle("_active");
            }
            if (!el.closest(".search-header__body._active") && !el.closest(".search-header__icon._active")) {
                ...
                if (searchBtn) searchBtn.classList.remove("_active");
            }
            if (el.closest(".tabs__button")) {
                const button = el.closest(".tabs__button");
                const tabsBlock = button.closest(".tabs");
                const buttons = tabsBlock.querySelectorAll(".tabs__button");
                if (!tabsBlock.classList.contains("tabs_filter")) buttons.forEach((item => {
                    if (item.classList.contains("tabs__button_active") && item != button) item.classList.remove("tabs__button_active");
                    button.classList.add("tabs__button_active");
                })); else button.classList.toggle("tabs__button_active");
            }
        }
        ...
        if (menuItems.length) menuItems.forEach((item => {
            ...
            item.addEventListener("mouseenter", (e => {
                if (!isMobile.any()) {
                    ...
                    subMenu.classList.add("menu__sub-menu_active");
                }
            }));
