 ...
 */
    ...
    (() => {
        ...
        function map(n) {
            ...
            function init() {
                var myMap = new ymaps.Map("map", {
                    controls: [],
                    center: [ 55.782031, 37.608212 ],
                    zoom: 16
                });
                let myPlacemark = new ymaps.Placemark([ 55.782031, 37.608212 ], {}, {
                    hasBalloon: false,
        ...
        da.init();
        const ta = document.querySelector(".comments__input textarea");
        const counter = document.querySelector(".comments__input span");
        ...
        document.querySelector(".product__status");
        function productOption(size) {
            if (size) {
                const status = size.dataset.status;
                const quantity = size.dataset.quantity;
                const button = document.querySelector(".product__button");
                const prodStatus = document.querySelector(".product__status");
                if ("in-stock" == status) {
                    button.disabled = false;
                    prodStatus.dataset.stat = "in-stock";
                    prodStatus.textContent = "В наличии " + quantity + "шт.";
                }
                if ("expectation" == status) {
                    button.disabled = true;
                    prodStatus.dataset.stat = "expectation";
                    prodStatus.textContent = "Примерно 1-2 недели";
                }
                if ("not-available" == status) {
                    button.disabled = true;
                    prodStatus.dataset.stat = "not-available";
                    prodStatus.textContent = "Нет в наличии";
                }
            } else if (!size) {
                const sizes = document.querySelectorAll(".size-product__item");
                const button = document.querySelector(".product__button");
                const prodStatus = document.querySelector(".product__status");
                let count = 0;
                let stat = false;
                for (let i = 0; i < sizes.length; i++) {
                    const quantity = sizes[i].dataset.quantity;
                    count += Number(quantity);
                }
                if (count > 0) {
                    button.disabled = false;
                    prodStatus.dataset.stat = "in-stock";
                    prodStatus.textContent = "В наличии " + count + "шт.";
                } else if (0 === count) {
                    for (let i = 0; i < sizes.length; i++) if ("expectation" == sizes[i].dataset.status) stat = true;
                    if (stat) {
                        prodStatus.dataset.stat = "expectation";
                        prodStatus.textContent = "Примерно 1-2 недели";
                    } else {
        ...
        if (document.querySelector(".size-product")) productOption();
        document.addEventListener("click", documentActions);
        function documentActions(e) {
            const el = e.target;
            if (el.closest(".teams__item-dota2")) {
                if (el.closest(".teams__wrap").classList.contains("_active")) el.closest(".teams__wrap").classList.remove("_active");
                document.querySelector(".teams__container").classList.add("_show");
                document.querySelector("#team-dota2").classList.add("_active");
            }
            if (el.closest(".teams__item-csgo")) {
                if (el.closest(".teams__wrap").classList.contains("_active")) el.closest(".teams__wrap").classList.remove("_active");
                document.querySelector(".teams__container").classList.add("_show");
                document.querySelector("#cs-go").classList.add("_active");
            }
            if (el.closest(".teams__item-valorant")) {
                if (el.closest(".teams__wrap").classList.contains("_active")) el.closest(".teams__wrap").classList.remove("_active");
                document.querySelector(".teams__container").classList.add("_show");
                document.querySelector("#valorant").classList.add("_active");
            }
            if (el.closest(".teams__back")) {
                if (el.closest(".teams__wrap").classList.contains("_active")) el.closest(".teams__wrap").classList.remove("_active");
                document.querySelector(".teams__container").classList.remove("_show");
                document.querySelector("#teams").classList.add("_active");
            }
            if (el.classList.contains("search__input")) if (!el.closest(".search").classList.contains("_active")) el.closest(".search").classList.add("_active");
            if (!el.closest(".search")) if (document.querySelector(".search").classList.contains("_active")) document.querySelector(".search").classList.remove("_active");
            if (el.closest(".top-panel__search-icon")) document.querySelector(".top-panel__search").classList.toggle("_show");
            if (el.classList.contains("filters__button-clear")) {
                const main = el.closest(".filters");
                const checks = main.querySelectorAll(".checkbox__input");
                checks.forEach((check => {
                    check.checked = false;
                }));
            }
            if (el.classList.contains("products__top-filters-button")) {
                const filters = document.querySelector(".filters");
                if (filters) filters.classList.add("_active");
                bodyLockToggle();
            }
            if (el.closest(".filters__close")) {
                const filters = document.querySelector(".filters");
                if (filters) if (filters.classList.contains("_active")) filters.classList.remove("_active");
                bodyLockToggle();
            }
            if (el.closest(".filters__button-apply")) {
                const filters = document.querySelector(".filters");
                if (filters) if (filters.classList.contains("_active")) filters.classList.remove("_active");
                bodyLockToggle();
            }
            if (el.classList.contains("top-banner__link")) {
                const main = el.closest(".top-banner");
                const block = main.querySelector(".top-banner__info");
                if (block) block.classList.toggle("_active");
            }
            if (el.closest(".size-product__item")) {
                const size = el.closest(".size-product__item");
                productOption(size);
            }
            if (el.closest(".checkout__submit")) formSubmit();
            if (el.closest(".form-checkout__map-button")) {
                const cover = el.closest(".form-checkout__map-cover");
                const map = document.querySelector("#map");
                if (cover.classList.contains("_active")) cover.classList.remove("_active");
