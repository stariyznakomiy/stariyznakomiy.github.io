...
*/
        var ScrollToPlugin_gsap, ScrollToPlugin_coreInitted, _window, ScrollToPlugin_docEl, ScrollToPlugin_body, ScrollToPlugin_toArray, ScrollToPlugin_config, ScrollToPlugin_ScrollTrigger, ScrollToPlugin_windowExists = function _windowExists() {
            ...
            if (ScrollToPlugin_windowExists() && ScrollToPlugin_gsap && "undefined" !== typeof document && document.body) {
                _window = window;
        ...
        var ScrollToPlugin = {
            ...
            init: function init(target, value, tween, index, targets) {
                ...
                "smooth" === ScrollToPlugin_gsap.getProperty(target, "scrollBehavior") && ScrollToPlugin_gsap.set(target, {
                    scrollBehavior: "auto"
                });
...
*/
        ...
        gsapWithCSS.registerPlugin(ScrollTrigger_ScrollTrigger, ScrollSmoother.ScrollSmoother, MotionPathPlugin, ScrollToPlugin);
        if (ScrollTrigger_ScrollTrigger.isTouch) document.documentElement.classList.add("touch");
        let vh = .01 * window.innerHeight;
        document.documentElement.style.setProperty("--vh", `${vh}px`);
        if (0 === ScrollTrigger_ScrollTrigger.isTouch) {
        ...
        const sections = Array.from(document.querySelector(".sections-content").children);
        window.addEventListener("load", (event => {
            setTimeout((() => {
                roundsWrap.setAttribute("data-step", "1");
            }), 1);
            if (1 !== ScrollTrigger_ScrollTrigger.isTouch) if (sections.length) sections.forEach((section => {
                let offsetNext, offsetPrev, sectionNext = section.nextElementSibling, sectionPrev = section.previousElementSibling;
                if (sectionNext) offsetNext = sectionNext.offsetTop;
                if (sectionPrev) offsetPrev = sectionPrev.offsetTop - (window.innerHeight - sectionPrev.offsetHeight);
                ScrollTrigger_ScrollTrigger.create({
                    trigger: section,
                    start: "top 20px",
                    end: "bottom 100%-=20px",
            ...
            let links = gsapWithCSS.utils.toArray("a.nav-link");
            links.forEach((a => {
            ...
            sliderButtons.forEach((sliderButton => {
                sliderButton.addEventListener("click", (e => {
                    e.preventDefault();
                    let index = Number(sliderButton.dataset.sliderNavigate) - 1;
                    const parent = sliderButton.closest("[data-slider]");
                    parent.dataset.sectionStep = sliderButton.dataset.sectionStep;
                    const rounds = document.querySelector(".rounds");
                    rounds.dataset.step = sliderButton.dataset.sectionStep;
                    const sliderItems = document.querySelectorAll("[data-slider-item]");
                    sliderItems.forEach((sliderItem => {
                        const childrens = Array.from(sliderItem.children);
                        for (let i = 0; i < childrens.length; i++) if (childrens[i].classList.contains("_active")) childrens[i].classList.remove("_active");
                        childrens[index].classList.add("_active");
                    }));
                }));
            }));
            function setStep(section) {
                roundsWrap.dataset.step = section.dataset.sectionStep;
            }
                ...
                animateBlocks.forEach((animateBlock => {
                    setTimeout((() => {
                        ScrollTrigger_ScrollTrigger.create({
                            ...
                            onEnter: () => {
                                animateBlock.classList.add("_animate");
                            }
                        });
                    }), 100);
                }));
        ...
        const contactButton = document.querySelector(".contact-sales-button");
        const sectionsLight = document.querySelectorAll(".section_light");
        sectionsLight.forEach((sectionLight => {
            ScrollTrigger_ScrollTrigger.create({
                ...
                onEnterBack: () => {
                    headerBlock.classList.add("theme-light");
                },
                onEnter: () => {
                    headerBlock.classList.add("theme-light");
                },
                onLeaveBack: () => {
                    headerBlock.classList.remove("theme-light");
                },
                onLeave: () => {
                    headerBlock.classList.remove("theme-light");
                }
            ...
            ScrollTrigger_ScrollTrigger.create({
                ...
                onEnterBack: () => {
                    contactButton.classList.add("theme-light");
                },
                onEnter: () => {
                    contactButton.classList.add("theme-light");
                },
                onLeaveBack: () => {
                    contactButton.classList.remove("theme-light");
                },
                onLeave: () => {
                    contactButton.classList.remove("theme-light");
                }
        ...
        function documentActionsClick(e) {
            ...
            if (bodyLockStatus && el.closest(".nav-link")) {
                bodyUnlock();
                document.documentElement.classList.remove("menu-open");
            }
        }
