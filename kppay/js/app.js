(() => {
    var __webpack_modules__ = {
        983: function(__unused_webpack_module, exports) {
            !function(e, t) {
                true ? t(exports) : 0;
            }(0, (function(e) {
                "use strict";
 ...
 */
        ...
        var sectionScroll_scroll = new smooth_scroll_polyfills_min;
        sectionScroll_scroll.animateScroll(0);
        function sectionScroll() {
            const sectionsWrapper = document.querySelector("[data-section-wrap]");
            new smooth_scroll_polyfills_min;
            const scrollSpeed = sectionsWrapper.dataset.scrollSpeed ? sectionsWrapper.dataset.scrollSpeed : 1e3;
            const sectionSlider = document.querySelector("[data-section-slider]");
            const prevSelector = ".section--prev";
            const nextSelector = ".section--next";
            if (sectionsWrapper && sectionsWrapper.children) {
                const firstSection = sectionsWrapper.firstElementChild;
                firstSection.classList.add("section", "section--active");
                if (firstSection.nextElementSibling) firstSection.nextElementSibling.classList.add("section", "section--next");
            }
            if (sectionSlider && sectionSlider.children) {
                const firstSlide = sectionSlider.firstElementChild;
                firstSlide.classList.add("section-slide-active");
            }
            window.addEventListener("wheel", (e => {
                sectionScrollAction(e, scrollSpeed);
            }));
            window.addEventListener("scroll", (e => {
                sectionLight();
            }));
            function sectionScrollAction(e, scrollSpeed) {
                let delta = -e.deltaY;
                if (delta > 0) {
                    if (!document.documentElement.classList.contains("lock")) sectionPrev(delta, scrollSpeed);
                } else if (!document.documentElement.classList.contains("lock")) sectionNext(delta, scrollSpeed);
            }
            function sectionPrev(delta, scrollSpeed) {
                ...
                if (prevSection) {
                    ...
                    if (sectionTop == windowTop || sectionTop > windowTop) if (flag) {
                        gotoBlock(prevSelector, false, scrollSpeed, -offsetTopPrev);
                        bodyLock(scrollSpeed);
                        sectionPrevClass(sectionActive, prevSection);
                        flag = false;
                        setTimeout((() => {
                            bodyUnlock(scrollSpeed);
                            flag = true;
                        }), scrollSpeed);
                    }
                }
            }
            function sectionNext(delta, scrollSpeed) {
                const sectionActive = document.querySelector(".section--active");
                ...
                if (nextSection) {
                    ...
                    if (sectionBottom == windowBottom || sectionBottom < windowBottom) if (flag) {
                        gotoBlock(nextSelector, false, scrollSpeed);
                        bodyLock(scrollSpeed);
                        sectionNextClass(sectionActive, nextSection);
                        flag = false;
                        setTimeout((() => {
                            flag = true;
                        }), scrollSpeed);
                        if (!nextSection.hasAttribute("data-section-slider")) setTimeout((() => {
                            bodyUnlock(scrollSpeed);
                        }), scrollSpeed);
                    }
                }
            }
            function sectionPrevClass(sectionActive, prevSection) {
                if (prevSection.previousElementSibling) prevSection.previousElementSibling.classList.add("section--prev");
                sectionActive.classList.remove("section--active");
                sectionActive.classList.add("section--next");
                prevSection.classList.remove("section--prev");
                prevSection.classList.add("section--active");
                if (sectionActive.nextElementSibling) sectionActive.nextElementSibling.classList.remove("section--next");
            }
            function sectionNextClass(sectionActive, nextSection) {
                if (sectionActive.previousElementSibling) sectionActive.previousElementSibling.classList.remove("section--prev");
                sectionActive.classList.remove("section--active");
                sectionActive.classList.add("section--prev");
                nextSection.classList.remove("section--next");
                nextSection.classList.add("section--active");
                if (nextSection.nextElementSibling) nextSection.nextElementSibling.classList.add("section--next");
            }
...
*/
        ...
        var ScrollSmoother = __webpack_require__(983);
        gsapWithCSS.registerPlugin(ScrollTrigger_ScrollTrigger, ScrollSmoother.ScrollSmoother);
