...
*/
        ...
        function countUp(target, startVal, endVal, decimals, duration, options) {
            ...
            var self = this;
            this.d = "string" === typeof target ? document.getElementById(target) : target;
            ...
            this.duration = 1e3 * duration || 2e3;
            this.easeOutExpo = function(t, b, c, d) {
                return c * (-Math.pow(2, -10 * t / d) + 1) * 1024 / 1023 + b;
            };
            this.count = function(timestamp) {
                if (null === self.startTime) self.startTime = timestamp;
                self.timestamp = timestamp;
                var progress = timestamp - self.startTime;
                self.remaining = self.duration - progress;
                if (self.options.useEasing) if (self.countDown) {
                    var i = self.easeOutExpo(progress, 0, self.startVal - self.endVal, self.duration);
                    self.frameVal = self.startVal - i;
                } else self.frameVal = self.easeOutExpo(progress, self.startVal, self.endVal - self.startVal, self.duration); else if (self.countDown) {
                ...
                if (progress < self.duration) self.rAF = requestAnimationFrame(self.count); else if (null != self.callback) self.callback();
            };
            this.start = function(callback) {
                self.callback = callback;
                if (!isNaN(self.endVal) && !isNaN(self.startVal)) self.rAF = requestAnimationFrame(self.count); else {
